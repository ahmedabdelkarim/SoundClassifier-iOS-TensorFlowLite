...
public protocol SoundClassifierDelegate: class {
    func soundClassifier(
        ...
        didInterpretProbabilities probabilities: [Float32]
    )
}
...
public class SoundClassifier {
    ...
    public func start(inputBuffer: [Int16]) {
        ...
        delegate?.soundClassifier(self, didInterpretProbabilities: probabilities)
    }
    
    // MARK: - Private Methods
    private func setupInterpreter() {
        guard let modelPath = Bundle.main.path(forResource: modelFileName, ofType: modelFileExtension)
        else {
            return
        }
